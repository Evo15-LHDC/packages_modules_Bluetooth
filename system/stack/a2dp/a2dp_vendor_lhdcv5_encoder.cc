...
typedef struct {
  ...
  uint32_t pcm_fmt;
  uint32_t frame_len;
  uint32_t max_target_bitrate_idx;
...
static void a2dp_vendor_lhdcv5_encoder_update(uint16_t peer_mtu,
  ...
  *p_config_updated = false;
  ...
  p_encoder_params->sample_rate = (uint32_t)a2dp_lhdc_encoder_cb.feeding_params.sample_rate;
  ...
  if (!A2DP_VendorGetFrameLenLhdcV5(&(p_encoder_params->frame_len), p_codec_info)){
    log::error( ": get frame length error");
    goto fail;
  }

  switch(p_encoder_params->frame_len) {
    case A2DP_LHDCV5_FRAME_LEN_2P5MS:
      frame_dur = LHDCV5_FRAME_2P5MS;
      break;
    case A2DP_LHDCV5_FRAME_LEN_5MS:
      frame_dur = LHDCV5_FRAME_5MS;
      break;
    case A2DP_LHDCV5_FRAME_LEN_10MS:
      frame_dur = LHDCV5_FRAME_10MS;
      break;
    default:
      break;
  }

  // get target quality index of max bitrate
  if (!A2DP_VendorGetMaxBitRateLhdcV5(&newValue_bt, codec_config.codec_specific_1)) {
    log::error( ": get max_target_bitrate_idx error");
    goto fail;
  }
  ...
  p_encoder_params->max_target_bitrate_idx = newValue_lib;

  // get target quality index of min bitrate
  if (!A2DP_VendorGetMinBitRateLhdcV5(&newValue_bt, codec_config.codec_specific_1) ) {
    log::error( ": get min_target_bitrate_idx error");
    goto fail;
  }
  if (!a2dp_vendor_lhdcv5_qualitymode_wrapper(&newValue_lib, newValue_bt)) {
    log::error( ": wrap mBR qualiity mode error");
  ...
  p_encoder_params->min_target_bitrate_idx = newValue_lib;
  ...
  // br128kbps flag
  if (!A2DP_VendorHasBr128kbpsFlagLhdcV5(&(p_encoder_params->isBr128kbpsEnabled), p_codec_info)){
  ...
  // get target quality index
  if ((codec_config.codec_specific_1 & A2DP_LHDCV5_QUALITY_CMD_MASK) == A2DP_LHDCV5_QUALITY_MAGIC_NUM) {
    //already configured
    newValue_bt = (codec_config.codec_specific_1 & A2DP_LHDCV5_QUALITY_MASK);
  } else {
    //not configured yet, give default mode: ABR
    newValue_bt = A2DP_LHDCV5_QUALITY_ABR;
  }
  if (!a2dp_vendor_lhdcv5_qualitymode_wrapper(&newValue_lib, newValue_bt)) {
  ...
  log::info( ": (encode param) sample_rate={} pcm_fmt={} frm_dur={} peer_mtu={} mtu={} "
      "MBRidx={} mBRidx={} LL={} LLess={} Raw={} newMmBR={} br128={} QM={}({})",
      ...
      p_encoder_params->max_target_bitrate_idx,         //max bitrate index
  ...
  lib_ret = lhdc_init_encoder(
      ...
      (uint32_t)p_encoder_params->isLLessEnabled);
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
    log::error( ": (lib_ret) lhdc_init_encoder {}",  lib_ret);
    goto fail;
  }

  // configure min/max bitrate after initialized to re-adjust target bitrate
  lib_ret = lhdc_set_max_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle, p_encoder_params->max_target_bitrate_idx);
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
    log::error( ": (lib_ret) set_max_bitrate {}",  lib_ret);
    goto fail;
  }

  lib_ret = lhdc_set_min_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle, p_encoder_params->min_target_bitrate_idx);
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
  ...
  lib_ret = lhdc_set_improved_low_bitrate(a2dp_lhdc_encoder_cb.lhdc_handle,
      p_encoder_params->isBr128kbpsEnabled);
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
    log::error( ": (lib_ret) lhdc_set_improved_low_bitrate error {}",  lib_ret);
    goto fail;
  }
...
void a2dp_vendor_lhdcv5_send_frames(uint64_t timestamp_us) {
  ...
  log::verbose( ": Sending {} frames per iteration, {} iterations",
...
static void a2dp_lhdcv5_get_num_frame_iteration(uint8_t* num_of_iterations,
  ...
  if (lib_ret != LHDCV5_FRET_SUCCESS) {
    log::error( ": get block size error {}",  lib_ret);
    return;
  }
  ...
  log::verbose( ": samples_per_frame={} pcm_bytes_per_frame={} nb_frame={}",
...
#endif
        log::verbose( ": nb_frame{} - written:{}, out_frames:{}",
...
static bool a2dp_lhdcv5_read_feeding(uint8_t* read_buffer, uint32_t *bytes_read) {
  ...
  log::verbose( ": expected read bytes {}, actual read bytes {}",
       read_size, nb_byte_read);

  //TODO: what to do if not alignment?
  if ((nb_byte_read % bytes_per_sample) != 0) {
    log::verbose( ": PCM data not alignment. The audio sample is shfit {} bytes!",
