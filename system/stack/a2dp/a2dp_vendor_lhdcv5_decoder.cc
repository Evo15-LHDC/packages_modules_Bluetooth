...
typedef struct {
  ...
  uint32_t    frame_duration_select;
...
bool a2dp_lhdcv5_decoder_save_codec_info (const uint8_t* p_codec_info)
{
  ...
  if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_1] &
    ...
    a2dp_lhdcv5_decoder_cb.sample_rate = LHDCV5BT_SAMPLE_RATE_44K;
  } else if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_1] &
      A2DP_LHDCV5_SAMPLING_FREQ_48000) {
    a2dp_lhdcv5_decoder_cb.sample_rate = LHDCV5BT_SAMPLE_RATE_48K;
  } else if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_1] &
      A2DP_LHDCV5_SAMPLING_FREQ_96000) {
    a2dp_lhdcv5_decoder_cb.sample_rate = LHDCV5BT_SAMPLE_RATE_96K;
  } else if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_1] &
      A2DP_LHDCV5_SAMPLING_FREQ_192000) {
    a2dp_lhdcv5_decoder_cb.sample_rate = LHDCV5BT_SAMPLE_RATE_192K;
  }

  // Bit Depth
  if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_2] &
      A2DP_LHDCV5_BIT_FMT_16) {
    a2dp_lhdcv5_decoder_cb.bits_per_sample = LHDCV5BT_BIT_DEPTH_16;
  } else if (p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_2] &
      A2DP_LHDCV5_BIT_FMT_24) {
    a2dp_lhdcv5_decoder_cb.bits_per_sample = LHDCV5BT_BIT_DEPTH_24;
  }

  // Frame Duration
  frame_dur =
      p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_3] & A2DP_LHDCV5_FRAME_LEN_MASK;
  if (frame_dur == A2DP_LHDCV5_FRAME_LEN_2P5MS) {
    a2dp_lhdcv5_decoder_cb.frame_duration = LHDCV5BT_FRAME_DUR_2P5MS;
  } else if (frame_dur == A2DP_LHDCV5_FRAME_LEN_5MS) {
    a2dp_lhdcv5_decoder_cb.frame_duration = LHDCV5BT_FRAME_DUR_5MS;
  } else if (frame_dur == A2DP_LHDCV5_FRAME_LEN_10MS) {
    a2dp_lhdcv5_decoder_cb.frame_duration = LHDCV5BT_FRAME_DUR_10MS;
  }

  // Frame Duration Select
  a2dp_lhdcv5_decoder_cb.frame_duration_select =
      p_codec_info[A2DP_LHDCV5_CODEC_INFO_ATTR_1] & A2DP_LHDCV5_FRAME_LEN_SELECT_MASK;
...
EXIT_FUNC:
  log::info(": sample_rate:{}",  a2dp_lhdcv5_decoder_cb.sample_rate);
  log::info(": bits_per_sample:{}", a2dp_lhdcv5_decoder_cb.bits_per_sample);
  log::info(": frame_duration:{}", a2dp_lhdcv5_decoder_cb.frame_duration);
  log::info(": frame_duration_select: 0x{:02X}", a2dp_lhdcv5_decoder_cb.frame_duration_select);
  log::info(": func_lless:{}", a2dp_lhdcv5_decoder_cb.func_lless);
  log::info(": func_lless_raw:{}", a2dp_lhdcv5_decoder_cb.func_lless_raw);

  log::info( ": {} ", (ret?"success":"failed"));
  return ret;
}
...
bool a2dp_vendor_lhdcv5_decoder_init(decoded_data_callback_t decode_callback) {
  ...
  if (a2dp_lhdcv5_decoder_cb.has_lhdc_handle == false &&
    ...
    lhdcdec_config.frame_duration = a2dp_lhdcv5_decoder_cb.frame_duration;
    lhdcdec_config.bit_rate = 0;
